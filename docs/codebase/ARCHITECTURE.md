...
src/main.ts
  ...
  -> validate root/collections/site fields
...
3. `TeamDataService` requests the relative JSON URL and validates the object,
   required collections, and core site fields.
...
- The JSON validator checks only selected root/site fields before casting the
  rest to `TeamData`; malformed nested records can reach templates.
...
- Shared styling is concentrated in a 1,878-line global stylesheet to stay
  within Angular component-style budgets.
