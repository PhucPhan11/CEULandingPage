...
| medium | JSON validation is shallow | `src/app/services/team-data.service.ts` validates selected root/site fields, then casts to `TeamData` | Malformed nested schedule, roster, result, or event records can fail during rendering | Add nested guards or a build-time schema check |
...
|-----------|---------------|-------|----------------|---------------|
| Global stylesheet is large | Shared layout/tokens avoid Angular component-style budget failures | `src/app/app.css` (1,878 lines measured during scan) | Changes can have broad selectors and become harder to isolate | Keep section boundaries clear; split only with a budget-safe styling plan |
| Content remains marked as sample | The project was implemented with placeholder/sample editorial data | `public/data/team-data.json`, `docs/content-guide.md` | Sample copy, roster, stats, or links could be published unintentionally | Team approval pass, then remove/replace `site.sampleNotice` |
...
| Outbound link destination changes | N/A | `src/app/components/site-footer/site-footer.component.html`, `schedule.component.html` | New-tab links use `rel="noopener"` | Link ownership/review remains manual |
...
| `src/app/app.css` | Global selectors and responsive rules affect the whole page | High churn and 1,878 lines | Make narrow section-scoped changes and check desktop/mobile behavior |
...
3. [ASK USER] Which current sample records, roster entries, and links are
