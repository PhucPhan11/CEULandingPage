...
| `src/app/components/` | Standalone page sections and the reusable contact icon | `src/app/app.ts`, `src/app/components/*` |
...
- Files and directories use kebab-case, for example
  `recent-events/recent-events.component.ts`.
...
- Shared model types use PascalCase interfaces and union types, such as
  `TeamData`, `RecentEvent`, `Language`, and `ScheduleKind`.
